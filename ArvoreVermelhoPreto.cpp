    ...
    if(avo != nullptr){
        ...
        if(getCor(pai) == VERMELHO && getCor(tio) == VERMELHO){
            ...
            if(getCor(pai) == VERMELHO && getCor(tio) == PRETO){
                    //Se av� menor que pai o n� est� na sub-�rvore direita
                if(strcmp(avo->getInfo()->getId(), pai->getInfo()->getId()) < 0){
                    //Se pai menor que n�, ele est� na sub-�rvore direita(rota��o simples a esquerda)
                    if(strcmp(pai->getInfo()->getId(), no->getInfo()->getId()) < 0){
                        ...
                        //Se pai n�o � menor que n�, ele est� na sub-�rvore esquerda(rota��o dupla a esquerda)
                        no->recolore();
                        avo->recolore();
                        // Se o no for a raiz, deve-se atualizar o ponteiro da raiz
                        if(avo->getPai() == nullptr){
                    ...
                    //Se av� maior que pai o n� est� na sub-�rvore esquerda
                    if(strcmp(pai->getInfo()->getId(), no->getInfo()->getId()) > 0){
                        //Se pai maior que n�, ele est� na sub-�rvore esquerda(rota��o simples a direita)
                        pai->recolore();
                        avo->recolore();
                        // Se o no for a raiz, deve-se atualizar o ponteiro da raiz
                        if(avo->getPai() == nullptr){
                        ...
                        //Se pai n�o � maior que n�, ele est� na sub-�rvore direita(rota��o dupla a direita)
                        no->recolore();
                        avo->recolore();
                        // Se o no for a raiz, deve-se atualizar o ponteiro da raiz
                        if(avo->getPai() == nullptr){
...
void ArvoreVermelhoPreto::insere(ReviewNode *info, int &comparacoes){
    ...
    if(vazia()){
        ...
        //Encontra a posi��o de inser��o e o pai
        while(aux != nullptr){
            pai = aux;
            comparacoes++;
        ...
        //Decide se � filho a direita ou esquerda
        comparacoes++;
